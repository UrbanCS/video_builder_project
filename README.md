...
Set `max_file_uploads` to at least **41** (recommended: **50**) in the hosting provider's effective PHP configuration. A limit of 20 silently truncates a 40-file request before the application receives it. Verify with a real browser upload: CLI and web PHP may use different settings, and persistent web workers may need their configuration reloaded. Recheck this setting after changing PHP versions or regenerating PHP Selector options. The application reports incomplete uploads without clearing the selection.
...
Syntax and regression checks do not replace an end-to-end test: upload 40 photos through a client account, customize the dedication, reload to verify draft restoration, render, and check the download through the closing card. Separately test owner/client permissions and email delivery. Existing MP4 files are not automatically regenerated by a code update.
